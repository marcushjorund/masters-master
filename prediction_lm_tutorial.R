...
for (b in 1:B){
  generated_data <- generate_data(n, psi, Y3L2coef, linear = TRUE, censoring = TRUE,test = TRUE)
  #Propensity scores
  uncensored_Y2 <- which(!is.na(generated_data$Y2))
  uncensored_Y3 <- which(!is.na(generated_data$Y3))
  p2_model <- glm(X2~X1+Y1+Y2+L1+L2+C, family = binomial("logit"),subset = uncensored_Y3,data = generated_data, na.action = na.exclude)
...
colnames(psi_dataframe_censoring) <- c("psi21", "psi31", "psi32")
colnames(psi_dataframe_censoring_naive) <- c("psi21", "psi31", "psi32")
...
ggplot(long_df, aes(x = Parameter, y = Estimate, fill = matrix)) +
  geom_boxplot() +
  scale_fill_manual(values = c("IPCW censoring estimation" = "blue", "Naive censoring estimation" = "red")) +
  theme_minimal() +
