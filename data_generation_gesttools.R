dataexamples <- function(n = 1000, t = 3, seed = NULL, Censoring = FALSE, psi = c(1,1,1)) {
  ...
  A.1 <- rbinom(n, 1, a.1)
  Y.1 <- rnorm(n, 1 + psi[1]*A.1 + L.1 + U, 1)
  
  L.2 <- rnorm(n, 1 + (A.1 / 2) + L.1 + U, 1)
  a.2 <- plogis(1 + 0.1 * L.2 + 0.1 * A.1)
  A.2 <- rbinom(n, 1, a.2)
  Y.2 <- rnorm(n, 1 + psi[1]*(A.1 / 2) + psi[2]*A.2 + L.1 + L.2 + U, 1)
  
  L.3 <- rnorm(n, 1 + (A.2 / 2) + L.2 + U, 1)
  a.3 <- plogis(1 + 0.1 * L.3 + 0.1 * A.2)
  A.3 <- rbinom(n, 1, a.3)
  Y.3 <- rnorm(n, 1 + psi[2]*(A.2 / 2) + psi[3]*A.3 + L.1 + L.2 + L.3 + U, 1)
  # End of study Outcome
  Y <- rnorm(n, 1 + psi[2](A.2 / 2) + psi[3]A.3 + L.1 + L.2 + L.3 + U, 1)
